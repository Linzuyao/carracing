classdef Map < handle
 ...
 %% test
            ...
            for i=1:n
                track2.outer(:,i)=track2.outer(:,i)+[40;53];
                track2.inner(:,i)=track2.inner(:,i)+[40;53];
            end
            
            for i=1:n
                xmax=ceil(max([track2.outer(1,i),track2.inner(1,i)]));
                xmin=floor(min([track2.outer(1,i),track2.inner(1,i)]));
                ymax=ceil(max([track2.outer(2,i),track2.inner(2,i)]));
                ymin=floor(min([track2.outer(2,i),track2.inner(2,i)]));
                for x=xmin:xmax
                    for y=ymin:ymax
                        self.map(y,x)=1;
                    end
