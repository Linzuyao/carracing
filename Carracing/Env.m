  ...
  %      pfmap % potential field map for the smart agents to avoid obstacles
    ...
    methods
     ...
     %       self.pfmap=PotentialField(self.map.occupyMap());
            ...
            agent=Agent(self.startPos.x,self.startPos.y,self.startPos.heading,self.startPos.vx,self.startPos.vy,self.startPos.omega,self.modelParam,....
                self.sensorInfo.scanRange,self.sensorInfo.scanAngle,self.obj_idx+1);
            agent.setSatLevel([self.agentInfo.dsat,self.agentInfo.deltasat,self.agentInfo.vtsat]);
            ...
            for i=1:length(sec)
                switch sec{i}
                    ...
                    case 'Agent'
                        ...
                        ai=self.agentInfo;
                        self.modelParam=ModelParam(ai.m,ai.Iz,ai.lf,ai.lr,...
                            ai.Cm1,ai.Cm2,ai.Cr0,ai.Cr2,ai.Br,ai.Cr,ai.Dr,ai.Bf,ai.Cf,ai.Df);
                    case 'System'
